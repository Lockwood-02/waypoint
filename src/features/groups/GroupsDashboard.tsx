      ...
      {showCreateGroup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4">
          <form onSubmit={handleCreateGroup} className="max-h-full w-full max-w-lg overflow-y-auto rounded-xl border border-white/10 bg-slate-950 p-6">
            ...
            <label className="mt-5 block text-sm">Name<input required maxLength={INPUT_LIMITS.groupName} value={groupName} onChange={(e) => setGroupName(e.target.value)} className="mt-2 w-full rounded-md border border-white/10 bg-slate-900 px-3 py-3" /></label>
            <label className="mt-4 block text-sm">Description<textarea maxLength={INPUT_LIMITS.groupDescription} value={groupDescription} onChange={(e) => setGroupDescription(e.target.value)} className="mt-2 min-h-24 w-full rounded-md border border-white/10 bg-slate-900 px-3 py-3" /></label>
