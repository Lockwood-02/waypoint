...
import { publicAppUrl } from '../../config/appConfig'
import {
  createGroup,
  createGroupTask,
  deleteGroupTask,
...
export function GroupsDashboard() {
  ...
  useEffect(() => {
    ...
    if (!token) return
    void joinGroup(token).then((response) => {
      if (response.error) setMessage(errorMessage(response.error))
      else {
  ...
  async function copyInvite() {
    if (!selectedGroup) return
    if (!publicAppUrl) {
      setMessage('Group invites are unavailable because VITE_APP_URL is not configured.')
      return
    }
    const url = new URL(publicAppUrl)
    url.searchParams.set('groupInvite', selectedGroup.invite_token)
  ...
  async function rotateInvite() {
    ...
    if (response.error) return setMessage(errorMessage(response.error))
    setMessage('A new invite link is ready. The previous link no longer works.')
    await loadGroups()
  }
      ...
      <div className="grid min-h-[32rem] overflow-hidden rounded-lg border border-white/10 bg-white/[0.06] shadow-2xl shadow-cyan-950/30 lg:grid-cols-[18rem_1fr]">
        <aside className="border-b border-white/10 p-4 lg:border-b-0 lg:border-r">
          <div className="flex items-center justify-between gap-3 px-2">
            ...
            <button onClick={() => setShowCreateGroup(true)} className="rounded-md bg-cyan-300 px-2.5 py-1.5 text-xs font-bold text-slate-950 transition hover:bg-cyan-200">Create group</button>
          </div>
        ...
        <div className="p-5 md:p-7">
          {selectedGroup ? <>
            <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h2 className="text-2xl font-bold">{selectedGroup.name}</h2><span className="rounded-full border border-white/15 px-2 py-1 text-xs capitalize text-slate-300">{selectedGroup.role}</span></div><p className="mt-2 text-sm text-slate-300">{selectedGroup.description || 'A shared place for your team’s tasks.'}</p></div><div className="flex flex-wrap gap-2"><button onClick={copyInvite} className="rounded-md border border-cyan-300/40 px-3 py-2 text-sm font-semibold text-cyan-100 hover:border-cyan-200">Copy invite link</button>{selectedGroup.role === 'owner' ? <button onClick={rotateInvite} className="rounded-md border border-white/15 px-3 py-2 text-sm text-slate-300 hover:text-white">Reset link</button> : <button onClick={handleLeave} className="rounded-md border border-rose-300/40 px-3 py-2 text-sm text-rose-100">Leave</button>}</div></div>
